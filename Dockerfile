...
RUN \
    openssl req \
        ...
        -nodes \
        -x509 \
        -subj "/C=US/ST=Alaska/L=Anchorage/O=Axiom Data Science/CN=erddap.example.com" \
...
VOLUME $CATALINA_HOME/content/erddap
VOLUME $ERDDAP_DATA
USER tomcat
EXPOSE 8080 8443
CMD ["catalina.sh", "run"]
